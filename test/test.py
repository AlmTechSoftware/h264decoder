...
def _compare_frames_eq(expected_frames, decoded_frames):
    assert len(expected_frames) == len(
        decoded_frames
    ), f"Number of decoded frames: {len(decoded_frames)} vs expected {len(expected_frames)}"

    for i, expected, decoded in zip(
        itertools.count(0), expected_frames, decoded_frames
    ):
        ...
        assert np.allclose(
            decoded, expected, atol=40
        ), f"Image max diff: {np.amax(np.abs(decoded-expected))} at frame {i}"
...
def _feed_decoder(decoder, buffer, max_feed_length, do_flush):
    if max_feed_length is None:
        ...
        buffers = np.array_split(
            np.frombuffer(buffer, dtype=np.uint8), len(buffer) // max_feed_length
        )
        decoded_frames = sum(
            (decoder.decode(buffer.tobytes()) for buffer in buffers), []
        )
    if do_flush:
...
def _decode_movie(filename, allow_retry=False, max_feed_length=None, do_flush=True):
    ...
    decoded_frames = _feed_decoder(
        decoder, content, max_feed_length=max_feed_length, do_flush=do_flush
    )
...
def _decode_mangled_movie(filename):
    ...
    content = np.concatenate(
        [
            np.frombuffer(content[:start_corrupt], dtype=np.uint8),
            np.uint8(rng.randint(0, 256, size=666)),
            np.frombuffer(content[start_corrupt:], dtype=np.uint8),
        ]
    ).tobytes()
...
def test_streaming_like():
    ...
    assert (
        len(decoded_frames) >= len(expected_frames) - LATENCY_FRAMES
    ), f"Number of decoded frames: {len(decoded_frames)} vs expected {len(expected_frames)}"
    expected_frames = expected_frames[: len(decoded_frames)]
    _compare_frames_eq(expected_frames, decoded_frames)

    # Feed only a few bytes at a time
    decoded_frames = _decode_movie(
        filename, allow_retry=False, do_flush=False, max_feed_length=42
    )
    assert (
        len(expected_frames) - LATENCY_FRAMES
        <= len(decoded_frames)
        <= len(expected_frames)
    ), f"Number of decoded frames: {len(decoded_frames)} vs expected {len(expected_frames)}"
    ...
    assert (
        len(expected_frames) - MAX_FRAMES_LOST_TO_CORRUPTION - LATENCY_FRAMES
        <= len(decoded_frames)
        <= len(expected_frames)
    )
...
def test_multithreading():
    ...
    assert [
        *sorted(only_the_worker_ids)
    ] != only_the_worker_ids, "Some degree of parallelism is expected. But it looks like one thread ran after the other."
    ...
    LATENCY_FRAMES = 5
    assert (
        len(expected_frames) - LATENCY_FRAMES <= len(frames1) <= len(expected_frames)
    ), f"Number of decoded frames: {len(frames1)} vs expected {len(expected_frames)}"
    assert (
        len(expected_frames) - LATENCY_FRAMES <= len(frames2) <= len(expected_frames)
    ), f"Number of decoded frames: {len(frames2)} vs expected {len(expected_frames)}"
