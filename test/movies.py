...
def create_frames() -> Tuple[np.ndarray, str]:
    ...
    N = 32
    SIGMA = 4.0 / N
    xgrid, ygrid = np.meshgrid(np.linspace(-1.0, 1.0, N), np.linspace(-1.0, 1.0, N))
    ...
    vals = np.exp(
        (
            -((xgrid[None, ...] - xs[:, None, None]) ** 2)
            - (ygrid[None, ...] - ys[:, None, None]) ** 2
        )
        / (SIGMA**2)
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        subprocess.check_call(
            [
                ...
                "-y",
                "-i",
                os.path.join(tmpdirname, r"frame%3d.png"),
                "-c:v",
                "libx264",
                "-f",
                "h264",
