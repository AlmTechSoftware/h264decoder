...
H264Decoder::H264Decoder() : pkt_{std::make_unique<AVPacket>()} {
...
ParseResult H264Decoder::parse(const ubyte *in_data, ptrdiff_t in_size) {
  auto nread = av_parser_parse2(parser, context, &pkt_->data, &pkt_->size,
                                in_data, in_size, 0, 0, AV_NOPTS_VALUE);
...
const AVFrame &ConverterRGB24::convert(const AVFrame &frame, ubyte *out_rgb) {
  ...
  context = sws_getCachedContext(context, w, h, (AVPixelFormat)pix_fmt, w, h,
                                 AV_PIX_FMT_RGB24, SWS_BILINEAR, nullptr,
                                 nullptr, nullptr);
  if (!context)
    throw H264DecodeFailure("cannot allocate context");

  // Setup framergb with out_rgb as external buffer. Also say that we want RGB24
  // output.
  av_image_fill_arrays(framergb->data, framergb->linesize, out_rgb,
                       AV_PIX_FMT_RGB24, w, h, 1);
  // Do the conversion.
  sws_scale(context, frame.data, frame.linesize, 0, h, framergb->data,
            framergb->linesize);
  framergb->width = w;
  framergb->height = h;
  return *framergb;
...
int ConverterRGB24::predict_size(int w, int h) {
  return av_image_fill_arrays(framergb->data, framergb->linesize, nullptr,
                              AV_PIX_FMT_RGB24, w, h, 1);
}
