...
public:
  GILScopedReverseLock() : state(nullptr) { unlock(); }

  ~GILScopedReverseLock() { lock(); }
...
py::object create_named_tuple_return_type() {
  ...
  return py::eval("typing.NamedTuple('Frame',[ ('data' , bytes), ('width' , "
                  "int), ('height' , int), ('rowsize' , int ) ])");
}
...
class PyH264Decoder {
  ...
  /* Extract frames from input stream. Stops at frame boundaries and returns the
   * number of consumed bytes in num_consumed.
   *
   * If a frame is completed, is_frame_available is set to true, and the
   * returned python tuple contains formation about the frame as well as the
   * frame buffer memory.
   *
   * Else, i.e. all data in the buffer is consumed, is_frame_available is set to
   * false. The returned tuple contains dummy data.
   */
  py::tuple decode_frame_impl(const ubyte *data, ssize_t len,
                              ssize_t &num_consumed, bool &is_frame_available);

public:
  /* Decoding style analogous to c/c++ way. Stop at frame boundaries.
   * Return tuple containing frame data as above as nested tuple, and an integer
   * telling how many bytes were consumed.  */
...
py::object PyH264Decoder::create_frame(py::bytes data, int w, int h,
                                       int rowsize) {
...
py::tuple PyH264Decoder::decode_frame_impl(const ubyte *data_in, ssize_t len,
  ...
  if (frame) {
    ...
    py::bytes py_out_str = py::reinterpret_steal<py::bytes>(
        PyBytes_FromStringAndSize(nullptr, out_size));
...
py::tuple PyH264Decoder::decode_frame(const py::bytes &data_in_str) {
  PyErr_WarnEx(PyExc_DeprecationWarning,
               "decode_frame() is deprecated and will be removed. Use decode() "
               "instead! It is easier to use and runs just as fast.",
               1);

  ssize_t len = PyBytes_Size(data_in_str.ptr());
  auto data_in =
      reinterpret_cast<const ubyte *>(PyBytes_AsString(data_in_str.ptr()));

  ssize_t num_consumed = 0;
  bool is_frame_available = false;
  auto frame =
      decode_frame_impl(data_in, len, num_consumed, is_frame_available);
...
py::list PyH264Decoder::flush() {
  ...
  do {
    ssize_t num_consumed = 0;
    auto frame =
        decode_frame_impl(nullptr, 0, num_consumed, is_frame_available);
    if (is_frame_available) {
      out.append(frame);
    }
...
PYBIND11_MODULE(h264decoder, m) {
  PyEval_InitThreads(); // need for release of the GIL
